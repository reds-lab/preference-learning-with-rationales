...
class Example:
    ...
    prompt_rat: str = ''                                        # prompt for the rationale explanation
...
def get_orca(split: str, human_prefix: str, human_suffix: str, assistant_prefix: str, assistant_suffix: str) -> Dataset:
    """
    Load the Orca without Rationales (binarized) dataset from Local Disk and convert it into to a Dataset.
    For this dataset, the SFT text is the preferred/chosen response.
    ...
    rank0_print(f'Loading Orca dataset ({split} split) from Local Disk...')
    dataset = datasets.load_from_disk('../orca')[split]
    if on_rank0():
        dataset = tqdm.tqdm(dataset, desc='Processing Orca Dataset')
...
def get_orcarat(split: str, human_prefix: str, human_suffix: str, assistant_prefix: str, assistant_suffix: str) -> Dataset:
    """
    Load the Orca with Rationales (binarized) dataset from Local Disk and convert it into to a Dataset.
    For this dataset, the SFT text is the preferred/chosen response.
    ...
    rank0_print(f'Loading Rationale Orca dataset ({split} split) from Local Disk...')
    dataset = datasets.load_from_disk('../rationalorca')[split]
    if on_rank0():
        dataset = tqdm.tqdm(dataset, desc='Processing Rationale Orca Dataset')

    data = Dataset('orcarat')

    for row in dataset:
        prompt = human_prefix + row['prompt'] + human_suffix + assistant_prefix
        
        prompt_rat = human_prefix  
        prompt_rat += f"Given the prompt:\n{row['prompt']}\n"
        prompt_rat += "Which of the following assistant's response is more preferable and strictly follows with the prompt question?\n"
        prompt_rat += f"Assistant 1:\"{row['chosen']}\"\nor\n"
        prompt_rat += f"Assistant 2:\"{row['rejected']}\"?\n"
        prompt_rat += "Provide a rationale of your answer." + assistant_prefix

        responses = [row['chosen'] + assistant_suffix, row['rejected'] + assistant_suffix, row['rationale'] + assistant_suffix]

        n_gen = data[prompt].num_generations()
        i, j, k = n_gen, n_gen + 1, n_gen + 2
        data[prompt].prompt = prompt
        data[prompt].prompt_rat = prompt_rat
        data[prompt].generations.extend(responses)
        data[prompt].pairs.append((i, j, k))
        data[prompt].sft_index = 0
        data[prompt].dataset_name = data.name
        data[prompt].truncation_mode = 'keep_start'
        data[prompt].remove_extra_spaces()
...
def get_orcaratgen(split: str, human_prefix: str, human_suffix: str, assistant_prefix: str, assistant_suffix: str) -> Dataset:
    """
    Load the Orca with General Rationales (binarized) dataset from Local Disk and convert it into to a Dataset.
    For this dataset, the SFT text is the preferred/chosen response.
    ...
    rank0_print(f'Loading General Rationale Orca dataset ({split} split) from Local Disk...')
    dataset = datasets.load_from_disk('../data/orcaratgen')[split]
    if on_rank0():
        dataset = tqdm.tqdm(dataset, desc='Processing General Rationale Orca Dataset')

    data = Dataset('orcaratgen')
    ...
    for row in dataset:
        ...
        # prompt_rat += f"Assistant 2:\"{row['rejected']}\"?\n"
        # prompt_rat += "Provide a rationale of your answer." + assistant_prefix
	# prompt_rat += "Which of the following assistant's response is more preferred and strictly follows with the prompt question?\n"
        prompt_rat += f"Response above:\"{row['chosen']}\"\nor\n"
        prompt_rat += f"Response below:\"{row['rejected']}\"?\n" + human_suffix + assistant_prefix
...
def get_orcaratgen_llama3_8it(split: str, human_prefix: str, human_suffix: str, assistant_prefix: str, assistant_suffix: str) -> Dataset:
    """
    Load the Orca with General Rationales (binarized) dataset from Local Disk and convert it into to a Dataset.
    For this dataset, the SFT text is the preferred/chosen response.
    ...
    rank0_print(f'Loading General Rationale Orca Llama3_8it dataset ({split} split) from Local Disk...')
    dataset = datasets.load_from_disk('../data/orcaratgen_llama3_8it')[split]
    if on_rank0():
        dataset = tqdm.tqdm(dataset, desc='Processing General Rationale Orca Dataset Llama 3 - 8b Instrcut')

    data = Dataset('orcaratgen_llama3_8it')
...
def get_orcaratspec(split: str, human_prefix: str, human_suffix: str, assistant_prefix: str, assistant_suffix: str) -> Dataset:
    """
    Load the Orca with Specific Rationales (binarized) dataset from Local Disk and convert it into to a Dataset.
    For this dataset, the SFT text is the preferred/chosen response.
    ...
    rank0_print(f'Loading Specific Rationale Orca dataset ({split} split) from Local Disk...')
    dataset = datasets.load_from_disk('../data/orcaratspec')[split]
    if on_rank0():
        dataset = tqdm.tqdm(dataset, desc='Processing Specific Rationale Orca Dataset')

    data = Dataset('orcaratspec')
...
def get_orcaratspec_llama3_8it(split: str, human_prefix: str, human_suffix: str, assistant_prefix: str, assistant_suffix: str) -> Dataset:
    """
    Load the Orca with Specific Rationales (binarized) dataset from Local Disk and convert it into to a Dataset.
    For this dataset, the SFT text is the preferred/chosen response.
    ...
    rank0_print(f'Loading Specific Rationale Orca Llama3 8ins dataset ({split} split) from Local Disk...')
    dataset = datasets.load_from_disk('../data/orcaratspec_llama3_8it')[split]
    if on_rank0():
        dataset = tqdm.tqdm(dataset, desc='Processing Specific Rationale Orca Dataset Llama3 8it')

    data = Dataset('orcaratspec_llama3_8it')
...
class RationalePairedPreferenceDataLoader(DataLoader):
    """
    Dataloader for losses that do require pairwise preferences with rationales (e.g., RDPO).
    """
    def __iter__(self):
        flat_data = []
        prompts = list(self.full_data.keys())
        ...
        while True:
            ...
            for example, (i,j,k) in flat_data:
                batch_element = {}
                batch_element.update(self.tokenize_batch_element(example.prompt, example.generations[i], example.truncation_mode, prefix='chosen'))
                batch_element.update(self.tokenize_batch_element(example.prompt, example.generations[j], example.truncation_mode, prefix='rejected'))
                batch_element.update(self.tokenize_batch_element(example.prompt_rat, example.generations[k], example.truncation_mode, prefix='rationale'))
                batch.append(batch_element)
