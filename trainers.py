...
class RationalePairedPreferenceTrainer(BasicTrainer):
    """A trainer for any loss that uses paired preference with Rationale, like RDPO."""
    def concatenated_inputs(self, batch: Dict[str, Union[List, torch.LongTensor]]) -> Dict[str, torch.LongTensor]:
        """Concatenate the chosen and rejected inputs into a single tensor. The first half is chosen outputs, the second half is rejected.
        ...
        Returns:
            A dictionary containing the concatenated inputs under the key 'concatenated_input_ids'.
        """
        max_length = max(batch['chosen_combined_input_ids'].shape[1], batch['rejected_combined_input_ids'].shape[1], batch['rationale_combined_input_ids'].shape[1])
        concatenated_batch = {}
        for k in batch:
            if k.startswith('chosen') and isinstance(batch[k], torch.Tensor):
                pad_value = -100 if 'labels' in k else 0
        ...
        for k in batch:
            if k.startswith('rationale') and isinstance(batch[k], torch.Tensor):
                pad_value = -100 if 'labels' in k else 0
    ...
    def forward(self, model: nn.Module, batch: Dict[str, Union[List, torch.LongTensor]]) -> Tuple[torch.FloatTensor, torch.FloatTensor]:
        ...
        chosen_n = batch['chosen_combined_input_ids'].shape[0]
        rejected_n = batch['rejected_combined_input_ids'].shape[0]
        rationale_n = batch['rationale_combined_input_ids'].shape[0]
        
        chosen_logps = all_logps[:chosen_n]
        
        rejected_logps = all_logps[chosen_n:chosen_n+rejected_n]
        
        rationale_logps = all_logps[chosen_n+rejected_n:]
        
        return chosen_logps, rejected_logps, rationale_logps
    ...
    def get_batch_metrics(self, batch: Dict[str, Union[List, torch.LongTensor]], mode: str=None):
        ...
        if self.reference_model is None:
            policy_chosen_logps, policy_rejected_logps, policy_rationale_logps = self.forward(self.policy, batch)
            losses, chosen_rewards, rejected_rewards = self.loss(policy_chosen_logps, policy_rejected_logps, policy_rationale_logps)
        else:
            policy_chosen_logps, policy_rejected_logps, policy_rationale_logps = self.forward(self.policy, batch)
            with torch.no_grad():
                reference_chosen_logps, reference_rejected_logps, reference_rationale_logps = self.forward(self.reference_model, batch)
            losses, chosen_rewards, rejected_rewards = self.loss(policy_chosen_logps, policy_rejected_logps, policy_rationale_logps, reference_chosen_logps, reference_rejected_logps, reference_rationale_logps)
        ...
        policy_chosen_logps = all_gather_if_needed(policy_chosen_logps.detach(), self.rank, self.world_size)
        policy_rejected_logps = all_gather_if_needed(policy_rejected_logps.detach(), self.rank, self.world_size)
        policy_rationale_logps = all_gather_if_needed(policy_rationale_logps.detach(), self.rank, self.world_size)
        all_devices_losses = all_gather_if_needed(losses.detach(), self.rank, self.world_size)
        ...
        metrics[f'logps_{mode}/rejected'] = policy_rejected_logps.float().cpu().numpy().tolist()
        metrics[f'logps_{mode}/chosen'] = policy_chosen_logps.float().cpu().numpy().tolist()
        metrics[f'logps_{mode}/rationale'] = policy_rationale_logps.float().cpu().numpy().tolist()
        metrics[f'loss/{mode}'] = all_devices_losses.float().cpu().numpy().tolist()

        del chosen_rewards, rejected_rewards, reward_accuracies, policy_chosen_logps, policy_rationale_logps, policy_rejected_logps, all_devices_losses
        if self.reference_model:
            del reference_chosen_logps, reference_rejected_logps, reference_rationale_logps
...
#         dpop_term = - 0.5 * max(0, reference_chosen_logps - policy_chosen_logps)
#         losses = -F.logsigmoid(self.config.loss.beta * logits + dpop_term
#         chosen_rewards = self.config.loss.beta * (policy_chosen_logps - reference_chosen_logps + dpop_term).detach()
#         rejected_rewards = self.config.loss.beta * (policy_rejected_logps - reference_rejected_logps).detach()

#         return losses, chosen_rewards, rejected_rewards


class RDPOTrainer(RationalePairedPreferenceTrainer):
       def loss(self,
             policy_chosen_logps: torch.FloatTensor,
             policy_rejected_logps: torch.FloatTensor,
             policy_rationale_logps: torch.FloatTensor,
             reference_chosen_logps: torch.FloatTensor,
             reference_rejected_logps: torch.FloatTensor,
             reference_rationale_logps: torch.FloatTensor) -> Tuple[torch.FloatTensor, torch.FloatTensor, torch.FloatTensor]:
        """Compute the DPO loss for a batch of policy and reference model log probabilities."""
        pi_logratios = policy_chosen_logps - policy_rejected_logps
        ref_logratios = reference_chosen_logps - reference_rejected_logps
        logits = pi_logratios - ref_logratios

        losses = -F.logsigmoid(self.config.loss.beta * logits) - F.logsigmoid(self.config.loss.gamma * policy_rationale_logps)
        chosen_rewards = self.config.loss.beta * (policy_chosen_logps - reference_chosen_logps).detach()
        rejected_rewards = self.config.loss.beta * (policy_rejected_logps - reference_rejected_logps).detach()

        return losses, chosen_rewards, rejected_rewards

class OddsPairedPreferenceTrainer(BasicTrainer):
    """A trainer for any loss that uses odds paired preference, like ORPO."""
    def concatenated_inputs(self, batch: Dict[str, Union[List, torch.LongTensor]]) -> Dict[str, torch.LongTensor]:
        """Concatenate the chosen and rejected inputs into a single tensor. The first half is chosen outputs, the second half is rejected.
    ...
    def get_batch_metrics(self, batch: Dict[str, Union[List, torch.LongTensor]], mode: str=None):
        ...
        # policy_chosen_logps, policy_rejected_logps, policy_chosen_logits, policy_rejected_logits, policy_nll_loss = self.forward(self.policy, batch)
        policy_chosen_logps, policy_rejected_logps, policy_nll_loss = self.forward(self.policy, batch)
        # losses, chosen_rewards, rejected_rewards, log_odds_ratio, log_odds_chosen = self.loss(policy_chosen_logps, policy_rejected_logps)
        losses, chosen_rewards, rejected_rewards = self.loss(policy_chosen_logps, policy_rejected_logps)
        ...
        all_devices_policynll = all_gather_if_needed(policy_nll_loss.detach(), self.rank, self.world_size)
        ...
        metrics[f'logps_{mode}/rejected'] = policy_rejected_logps.float().cpu().numpy().tolist()
        metrics[f'logps_{mode}/chosen'] = policy_chosen_logps.float().cpu().numpy().tolist()
        metrics[f'loss_{mode}/nll'] = all_devices_policynll.float().cpu().numpy().tolist()
        # metrics[f"logits_{mode}/rejected"] = policy_rejected_logits.float().cpu().numpy().tolist()
        # metrics[f"logits_{mode}/chosen"] = policy_chosen_logits.float().cpu().numpy().tolist()
        # metrics[f"log_odds_{mode}_ratio"] = log_odds_ratio.cpu().numpy().tolist()
        # metrics[f"log_odds_{mode}_chosen"] = log_odds_chosen.cpu().numpy().tolist()

        metrics[f'loss/{mode}'] = all_devices_losses.float().cpu().numpy().tolist()
        # metrics[f'loss/{mode}'] = all_devices_losses.float().cpu().numpy().tolist()

        del chosen_rewards, rejected_rewards, reward_accuracies, policy_chosen_logps, policy_rejected_logps, all_devices_losses, all_devices_policynll

        return loss, metrics
    
class ORPOTrainer(OddsPairedPreferenceTrainer):
       def loss(self,
             policy_chosen_logps: torch.FloatTensor,
        ...
        losses = self.config.loss.beta * ratio
...
class RationaleOddsPairedPreferenceTrainer(BasicTrainer):
    """A trainer for any loss that uses odds paired preference, like R-ORPO."""
    def concatenated_inputs(self, batch: Dict[str, Union[List, torch.LongTensor]]) -> Dict[str, torch.LongTensor]:
        """Concatenate the chosen and rejected inputs into a single tensor. The first half is chosen outputs, the second half is rejected.
    ...
    def forward(self, model: nn.Module, batch: Dict[str, Union[List, torch.LongTensor]]) -> Tuple[torch.FloatTensor, torch.FloatTensor]:
        ...
        chosen_nll_loss = cross_entropy_loss(all_logits[:chosen_n], labels[:chosen_n])

        # chosen_logits = all_logits[:chosen_n]
        # rejected_logits = all_logits[chosen_n:chosen_n+rejected_n]
        # ratinale_logits = all_logits[chosen_n+rejected_n:]

        # return chosen_logps, rejected_logps, chosen_logits, rejected_logits, chosen_nll_loss
        return chosen_logps, rejected_logps, rationale_logps, chosen_nll_loss
    ...
    def get_batch_metrics(self, batch: Dict[str, Union[List, torch.LongTensor]], mode: str=None):
        ...
        # policy_chosen_logps, policy_rejected_logps, policy_chosen_logits, policy_rejected_logits, policy_nll_loss = self.forward(self.policy, batch)
        policy_chosen_logps, policy_rejected_logps, policy_rationale_logps, policy_nll_loss = self.forward(self.policy, batch)
        # losses, chosen_rewards, rejected_rewards, log_odds_ratio, log_odds_chosen = self.loss(policy_chosen_logps, policy_rejected_logps)
        losses, chosen_rewards, rejected_rewards = self.loss(policy_chosen_logps, policy_rejected_logps, policy_rationale_logps)
        ...
        del chosen_rewards, rejected_rewards, reward_accuracies, policy_chosen_logps, policy_rejected_logps, all_devices_losses, all_devices_policynll, policy_rationale_logps

        return loss, metrics
    
    
class RORPOTrainer(RationaleOddsPairedPreferenceTrainer):
       def loss(self,
             policy_chosen_logps: torch.FloatTensor,
             policy_rejected_logps: torch.FloatTensor,
             policy_rationale_logps: torch.FloatTensor) -> Tuple[torch.FloatTensor, torch.FloatTensor, torch.FloatTensor]:
        """Compute the R-ORPO loss for a batch of policy and reference model log probabilities."""
        ...
        # OR Loss + Rationale Loss (Subtracted in Get Batch Metrics)
        losses = self.config.loss.beta * ratio + F.logsigmoid(self.config.loss.gamma * policy_rationale_logps)
...
class SimpleRORPOTrainer(RationaleOddsPairedPreferenceTrainer):
       def loss(self,
             policy_chosen_logps: torch.FloatTensor,
             policy_rejected_logps: torch.FloatTensor,
             policy_rationale_logps: torch.FloatTensor) -> Tuple[torch.FloatTensor, torch.FloatTensor, torch.FloatTensor]:
        ...
        # OR Loss + Rationale Loss (Subtracted in Get Batch Metrics)
        losses = self.config.loss.beta * ratio + self.config.loss.gamma * policy_rationale_logps
...
class KTOTrainer(UnpairedPreferenceTrainer):
    ...
    def forward(self, model: nn.Module, batch: Dict[str, Union[List, torch.LongTensor]]) -> Tuple[torch.FloatTensor, torch.FloatTensor, torch.FloatTensor]:
        ...
        chosen_logps = target_logps[chofsen_idx, ...]
